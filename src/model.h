...
private:
    ...
    Mesh processMesh(aiMesh *mesh, const aiScene *scene)
    {
        ...
        for(unsigned int i = 0; i < mesh->mNumFaces; i++)
        {
			...
			//cout << "faceIndices: " << face.mNumIndices << endl; 3
    ...
    vector<Texture2D> loadMaterialTextures(aiMaterial *mat, aiTextureType type, string typeName)
    {
        ...
        for(unsigned int i = 0; i < mat->GetTextureCount(type); i++)
        {
            ...
            {   // if texture hasn't been loaded already, load it
				...
				//cout << filename << ":" << typeName <<endl;
				Texture2D texture(filename.c_str(), gammaCorrection, GL_REPEAT, GL_REPEAT, GL_LINEAR_MIPMAP_LINEAR, GL_LINEAR);
                texture.type = typeName;
                texture.path = str.C_Str();
                textures.push_back(texture);
				// store it as texture loaded for entire model, to ensure we won't unnecesery load duplicate textures.
				// textures_loaded saves all meshes' textures. But textures saves only the current mesh.
