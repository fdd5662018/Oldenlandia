...
private:
	...
	* So only three values are required to store the inertia tensor. Three values
	...
	* that is performed to get a diagnoal inertia tensor
	*/
	Quaternionr m_q_mat;
	...
	* Important for the transformation from world in bodyspace and vice versa.
	...
	*/
	...
	// transformation required to transform a point to local space or vice versa.
...
public:
	...
	void initBody(const Real mass, const Vector3r &x,
		...
		const VertexData &vertices, const Vector3r &scale = Vector3r(1.0, 1.0, 1.0))
	{
		setMass(mass);
		m_x = x;
		m_x0 = x;
